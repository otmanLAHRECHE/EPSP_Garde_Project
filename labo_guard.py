...
import export_labo_guard
import laboratoire
from dialogs import Saving_progress_dialog, CustomDialog
from threads import Thread_load_guards_laboratoire, Thread_create_laboratoire_guard, ThreadAutoGuard
from widgets import Chose_worker
import os
basedir = os.path.dirname(__file__)

class LaboGuardUi(QtWidgets.QMainWindow):
    def __init__(self, month, year):
        ...
        self.ttl.setText("Planing de garde agents de labo mois " + str(m) + "/" + str(self.year) + ":")
        self.load_med()
        self.load_guards()
    ...
    def load_guards(self):
        ...
        self.thr2 = Thread_load_guards_laboratoire(self.num_days, self.month, self.year)
    ...
    def save_(self):
        ...
        self.dialog = Saving_progress_dialog()
        self.dialog.show()
        self.thr = Thread_create_laboratoire_guard(self.num_days, self.month, self.year, self.table)
    ...
    def export(self):
        self.want_to_close = True
        self.next_page = export_labo_guard.ExportLaboGuard(self.month, self.year)
        self.close()
        self.next_page.show()
    ...
    def auto_(self):
        ...
        if len(auto) == 0:
            ...
            self.dialog = Saving_progress_dialog()
            self.dialog.show()
            self.thr3 = ThreadAutoGuard(self.num_days, self.month, self.year, "labo", self.table, auto)
