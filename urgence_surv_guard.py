...
import export_urgence_surv
import urgence_inf
from dialogs import Saving_progress_dialog, CustomDialog
from threads import Thread_load_guards_surv_urgences, Thread_create_urgence_surv_guard, ThreadAutoGuard
from widgets import Chose_worker

basedir = os.path.dirname(__file__)

class UrgenceSurvGuardUi(QtWidgets.QMainWindow):
    def __init__(self, month, year):
        super(UrgenceSurvGuardUi, self).__init__()
        uic.loadUi(os.path.join(basedir, 'ui', 'urgence_surv_guard.ui'), self)
        ...
        self.ttl.setText("Planing de garde des survients mois " + str(m) + "/" + str(self.year) + ":")
        self.load_med()
        self.load_guards()
    ...
    def auto_(self):
        ...
        if len(auto) == 0:
            ...
            self.dialog = Saving_progress_dialog()
            self.dialog.show()
            self.thr3 = ThreadAutoGuard(self.num_days, self.month, self.year, "urgence_surv", self.table, auto)
