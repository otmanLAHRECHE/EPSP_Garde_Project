...
from threads import Thread_load_guards_pharmacie, Thread_create_pharmacie_guard, ThreadAutoGuard
...
class PharmGuardUi(QtWidgets.QMainWindow):
    ...
    def load_guards(self):
        ...
        self.thr2 = Thread_load_guards_pharmacie(self.num_days, self.month, self.year)
    ...
    def save_(self):
        ...
        self.dialog = Saving_progress_dialog()
        self.dialog.show()
        self.thr = Thread_create_pharmacie_guard(self.num_days, self.month, self.year, self.table)
    ...
    def export(self):
        self.want_to_close = True
        self.next_page = export_pharm_guard.ExportPharmGuard(self.month, self.year)
        self.close()
        self.next_page.show()
    ...
    def auto_(self):
        ...
        if len(auto) == 0:
            ...
            self.dialog = Saving_progress_dialog()
            self.dialog.show()
            self.thr3 = ThreadAutoGuard(self.num_days, self.month, self.year, "pharm", self.table, auto)
