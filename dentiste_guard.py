...
from threads import Thread_load_guards_dentiste, Thread_create_dentiste_guard, ThreadAutoGuard
...
class DentisteGuardUi(QtWidgets.QMainWindow):
    def __init__(self, month, year):
        super(DentisteGuardUi, self).__init__()
        uic.loadUi(os.path.join(basedir, 'ui', 'guard_dentiste.ui'), self)
        ...
        self.ttl.setText("Planing de garde dentistes mois " + str(m) + "/" + str(self.year) + ":")
        self.load_med()
        self.load_guards()
    ...
    def load_guards(self):
        ...
        self.thr2 = Thread_load_guards_dentiste(self.num_days, self.month, self.year)
    ...
    def auto_(self):
        ...
        if len(auto) == 0:
            ...
            self.dialog = Saving_progress_dialog()
            self.dialog.show()
            self.thr3 = ThreadAutoGuard(self.num_days, self.month, self.year, "dentiste", self.table, auto)
