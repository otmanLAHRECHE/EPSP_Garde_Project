...
from threads import Thread_load_guards_infirmier, Thread_create_infirmier_guard, ThreadAutoGuard
...
class InfirmierGuardUi(QtWidgets.QMainWindow):
    ...
    def auto_(self):
        ...
        if len(auto) == 0:
            ...
            self.dialog = Saving_progress_dialog()
            self.dialog.show()
            self.thr3 = ThreadAutoGuard(self.num_days, self.month, self.year, "dentiste_inf", self.table, auto)
